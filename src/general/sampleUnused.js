...
export default function FriendScreen({navigation}) {
  ...
  useEffect(() => {
    const getFollowingPosts = followingList => {
      ...
      db.collection('WDA_Posts')
        .where('authory', 'in', followingList)
        .onSnapshot(snapshot => {
  ...
  const DisplayAnImageWithStyle = ({item}) => {
    ...
    return (
      <View id={id} style={styles.container}>
        <Image style={styles.tinyLogo} source={{uri: imageLink}} />
        <Text>{phrase} by</Text>
  ...
  return (
    <View>
      ...
      <Text style={styles.text}>
        {' '}
        Check out what your friends posted recently{' '}
      </Text>
      <FlatList
